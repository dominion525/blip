...
The settings window also picks the effect and turns on launch at login.
...
Most values that are not in the settings window live in `enum Config` in Sources/Blip/main.swift. Edit them and run build.sh again. The intervals for re-checking the Input Monitoring permission live in Sources/Blip/ModifierTapMonitor.swift instead.
...
Sources/Blip/LoginItem.swift     SMAppService wrapper for launch at login
Sources/Blip/Localization.swift  String lookup
Sources/Blip/Info.plist          Written by XcodeGen from the info: block in project.yml
