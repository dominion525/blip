...
final class AppDelegateMenuTests: XCTestCase {
    func testStatusMenuHasTheExpectedItemsInOrder() {
        ...
        XCTAssertEqual(menu.items[5].title, L("menu.restart"))
        XCTAssertEqual(menu.items[6].title, L("menu.quit"))
    }
    ...
    func testStatusMenuShortcuts() {
        ...
        XCTAssertEqual(menu.items[5].action, #selector(AppDelegate.restart))
        XCTAssertEqual(menu.items[6].keyEquivalent, "q")
        XCTAssertEqual(menu.items[6].action, #selector(NSApplication.terminate(_:)))
    }
