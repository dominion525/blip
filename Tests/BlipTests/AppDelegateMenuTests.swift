...
final class AppDelegateMenuTests: XCTestCase {
    func testStatusMenuHasTheExpectedItemsInOrder() {
        ...
        XCTAssertEqual(menu.items[5].title, L("menu.checkForUpdates"))
        XCTAssertEqual(menu.items[6].title, L("menu.restart"))
        XCTAssertTrue(menu.items[7].isSeparatorItem)
        XCTAssertEqual(menu.items[8].title, L("menu.quit"))
    }
    ...
    func testStatusMenuShortcuts() {
        ...
        XCTAssertEqual(menu.items[5].keyEquivalent, "")
        XCTAssertEqual(menu.items[5].action, #selector(AppDelegate.checkForUpdates))
        XCTAssertEqual(menu.items[6].keyEquivalent, "")
        XCTAssertEqual(menu.items[6].action, #selector(AppDelegate.restart))
        XCTAssertEqual(menu.items[8].keyEquivalent, "q")
        XCTAssertEqual(menu.items[8].action, #selector(NSApplication.terminate(_:)))
    }
